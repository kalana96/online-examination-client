...
function AddClass() {
  ...
  const [subjects, setSubjects] = useState([]); // State to store the complete list of available subjects fetched from the backend
  const [filteredSubjects, setFilteredSubjects] = useState([]); // State to store the filtered list of subjects based on the search term
  const [showSubjectDropdown, setShowSubjectDropdown] = useState(false); // State to control the visibility of the subject dropdown
  ...
  const fetchData = async () => {
    try {
      ...
      if (subjectRes.code === "00") {
        ...
        setFilteredSubjects(subjectRes.content); // Initially show all
      } else {
    ...
    try {
      const classRes = await ClassService.getAllClass(token);
      if (classRes.code === "00") {
  ...
  // Enhanced addSubject function with logging
  const addSubject = (subject) => {
    if (!formData.selectedSubjects.some((s) => s.id === subject.id)) {
      // console.log("Adding subject:", subject);
      setFormData((prev) => {
        const newSelectedSubjects = [...prev.selectedSubjects, subject];
        // console.log("Updated selectedSubjects:", newSelectedSubjects);
        return {
          ...prev,
  ...
  //Function to remove subjects from the formData.selectedSubjects list
  const removeSubject = (id) => {
    setFormData((prev) => ({
      ...prev,
      selectedSubjects: prev.selectedSubjects.filter((s) => s.id !== id),
    }));
  };
  ...
  const resetDropdown = () => {
    ...
    setFilteredSubjects(subjects); // Reset the filtered list to full list
    setShowSubjectDropdown(false); // Hide the dropdown
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        subjectIds: formData.selectedSubjects.map((subject) => subject.id),
        userDetails: {
              ...
              {formData.selectedSubjects.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                  ...
                  <p className="text-sm text-gray-400">
                    Search and add subjects from above
                  </p>
                </div>
              ) : (
                <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
