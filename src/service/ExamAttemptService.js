...
class ExamAttemptService {
  constructor() {
    ...
    this.apiPath = "/api/v1/common/exam-attempts";
   ...
   */
  async getExamAttemptsById(attemptId) {
    try {
      const response = await this.axiosInstance.get(
        `${this.apiPath}/${attemptId}`
      );
      return response.data;
    } catch (error) {
      throw this.handleError(error, "Failed to get the exam attempt");
    }
  }

  /**
   * Get exam attempt list with questions and answers
   * @param {number} examId - The exam ID
   * @returns {Promise<Object>} ExamTakeDTO object
   */
  async getExamAttemptsByExamId(examId) {
    try {
      const response = await this.axiosInstance.get(
        `${this.apiPath}/exam/${examId}`
      );
      return response.data;
    } catch (error) {
      throw this.handleError(error, "Failed to get exam attempts");
    }
  }
