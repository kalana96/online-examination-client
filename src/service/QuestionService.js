...
// const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || 'http://localhost:1010/api/v1/teacher/question';
const API_BASE_URL = `${import.meta.env.VITE_API_BASE_URL}/api/v1/teacher/question`;
...
class QuestionService {
  ...
  async getQuestionsByExam(examId, token) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/getQuestionsByExam/${examId}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
